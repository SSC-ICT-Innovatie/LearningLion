...
class Querier:
    ...
    def ask_question(self, question: str) -> Tuple[Dict[str, Any], List[float]]:
        ...
        """
        prompt_helper = """Answer the following question in Dutch with the corresponding documents. If the answer is not in the documents, just say that the data is not in the documents."""
        ...
        if settings.RETRIEVAL_METHOD == "answer_and_question":
            ...
            custom_documents = sorted(unique_documents.values(), key=lambda x: x[1], reverse=True)
            
            response = self.chain.invoke({"question": f"{prompt_helper} {question}", "chat_history": self.chat_history}, custom_documents=custom_documents)
            
            # if no chunk qualifies, overrule any answer generated by the LLM with message below
            _, first_score = custom_documents[0]
        else:
            response = self.chain.invoke({"question": f"{prompt_helper} {question}", "chat_history": self.chat_history})
            response["source_documents"] = most_similar_docs
            _, first_score = most_similar_docs[0]
